...
class Decoder(nn.Module):
    ...
    def forward(self, tensorFirst, tensorSecond, objectPrevious):
        ...
        if objectPrevious is None:  # Top of the pyramid! no need to upsample flow and feature map.
            ...
            # F.pad(, [0, pW, 0, pH])
            tensorFlow = F.interpolate(objectPrevious['tensorFlow'], size=tensorFirst.shape[-2:], mode='bilinear')
            tensorFeat = torch.zeros_like(tensorFlow).to(tensorFlow.device)
            # tensorFlow = pad_as_size(self.moduleUpflow(objectPrevious['tensorFlow']),
            #                          output_size=tensorFirst.shape[-2:])
